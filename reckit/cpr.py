...
class CPRSampler(object):
    def __init__(self, dataset, sample_ratio, sample_rate, batch_size=1024, n_thread=4, k_interact=None, max_k_interact=3, drop_last=False):
        ...
        self.num_ratings = dataset.num_train_ratings
        ...
        u_interacts, i_interacts = [], []
        for u, items in enumerate(self.train):
            for i in items:
                u_interacts.append(u)
                i_interacts.append(i)
        self.u_interacts = np.array(u_interacts, dtype=np.int32)
        self.i_interacts = np.array(i_interacts, dtype=np.int32)
        ...
        self.batch_choice_sizes = 3 * self.batch_sample_sizes  # gamma默认为3
