...
int eval_one_user(float *ratings, int rating_len, const unordered_set<int> &truth,
                  const vector<int> &metric, const vector<int> &item_degrees, int top_k, float *result_pt)
{
    ...
    std::partial_sort_copy(index.begin(), index.end(), topk_rank.begin(), topk_rank.end(),
                           [&ratings](int &x1, int &x2) -> bool
                           { return ratings[x1] > ratings[x2]; });
    ...
    for (unsigned int i = 0; i < metric.size(); i++)
    {
        ...
        metric_dict[metric[i]](top_k_cut, truth, item_degrees, r_pt);
    }
...
void cpp_evaluate_matrix(float *rating_matrix, int rating_len, vector<unordered_set<int>> &test_items,
                         vector<int> metric, vector<int> &item_degrees, int top_k, int thread_num, float *results_pt)
{
    ...
    for (unsigned int i = 0; i < test_items.size(); i++)
    {
        ...
        sync_results.emplace_back(pool.enqueue(eval_one_user, rating_pt, rating_len, truth, metric, item_degrees, top_k, r_pt));
    }
